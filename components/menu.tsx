...
export default function Menu() {
    ...
    return (
        <div className="flex flex-col-reverse items-end">
            <button className="items-center outline-none text-xl font-fira-code font-bold m-4 mt-0 h-7 w-7 bg-white dark:bg-dark-background rounded-lg bg-opacity-85 dark:bg-opacity-85 hover:scale-125" onClick={(e)=>{setOpen(!open)}}>{open ? "-": "+"}</button>
            <AnimatePresence>
                {open && (
                        <motion.div initial={{ x: 200 }}
                                    ...
                                    exit={{ x: 200 }}
                                    className="flex flex-col justify-end items-end my-2 mx-2 bg-white dark:bg-dark-background rounded-lg bg-opacity-85 dark:bg-opacity-85">
